...
describe("Provider: github-pull-requests", () => {
    ...
    global.fetch = vitest.fn().mockImplementation(
      () =>
        Promise.resolve({
          ...
          json: () =>
            Promise.resolve([
              { id: 56, title: "Cat's Meow", source_branch: "mr/hiss", head: { sha: "aze" } },
                { id: 54, title: "Zombie guts", source_branch: "mr/gutsofdatzombie", head: { sha: "eza" } },
            ]),
        }) as any
    )
  })
  ...
  test("should test if applicable", () => {
    ...
    expect(provider.isApplicable({ ...config, baseUrl: null })).eq(false)
  })
